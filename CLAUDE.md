...
2. `.docs/NYAACASTER-DEV-GUIDE.md` — NyaaCaster 开发风格 & 技术栈指南（命名规范、架构模式、跨模型矩阵调度策略）
3. `CLAUDE.md`（本文件）— 项目规则
...
- 使用 `rebuild.py` 脚本（Python 3）进行 Docker 构建和重启
- 支持 `--no-cache` 参数强制无缓存构建
- 流程：build → up -d → 清理悬空镜像 → 状态报告
- 禁止编写 `.ps1` / `.sh` 自动化脚本（工作空间统一用 Python）
...
4. 根据项目名重命名全局标识（package.json、docker-compose、rebuild.py、CLAUDE.md、version.ts）
5. 确定项目技术堆栈，拉取对应依赖
6. 更新 Docker 配置（从 meta.json 读取容器名和端口）
7. 根据实际需求修改 `rebuild.py` 脚本
